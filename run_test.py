...
if __name__ == "__main__":
    ...
    if args.all:
        ...
        for case in step_cases:
            case.run(simplex_step)
        for case in init_cases:
            case.run(simplex_init)
        for case in method_cases:
            case.run(simplex_method)
    elif args.test:
        assert (args.test >= 1 and args.test <= 9), "Invalid test number!"
        if args.test in range(1, 6):
            step_cases[args.test - 1].run(simplex_step)
        elif args.test in range(6, 9):
            init_cases[args.test - 6].run(simplex_init)
        else:
            method_cases[args.test - 9].run(simplex_method)
    elif args.function:
